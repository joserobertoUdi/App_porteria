...
var refreshTokenExpireDays = builder.Configuration.GetValue<double?>("JWT_REFRESH_TOKEN_DAYS")
    ?? builder.Configuration.GetValue<double?>("Jwt:RefreshTokenExpireDays")
    ?? 7;
...
var sharepointBaseUrl = builder.Configuration.GetValue<string>("SHAREPOINT_BASE_URL")
    ?? "http://10.1.210.10/SharepointApi";
var sharepointProviderKey = builder.Configuration.GetValue<string>("PROVIDER_KEY")
    ?? throw new InvalidOperationException("PROVIDER_KEY no configurada.");

builder.Services.Configure<SharepointSettings>(options =>
{
    options.BaseUrl = sharepointBaseUrl;
    options.ProviderKey = sharepointProviderKey;
    options.Contenedor = "parqueo";
    options.EntidadOrigen = "PorteriaEntrada";
});
