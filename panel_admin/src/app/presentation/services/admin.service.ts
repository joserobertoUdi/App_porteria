...
export class AdminService {
  ...
  /**
   * Auditoría de guardia por día y turno: aprobaciones (portería + parqueo)
   * de un operador. `fecha` opcional en formato `yyyy-MM-dd`.
   */
  obtenerAprobacionesOperador(id: number, fecha?: string): Observable<AprobacionesOperador> {
    return this.repo.obtenerAprobacionesOperador(id, fecha).pipe(
      catchError((err) => {
        this.toast.error(extraerMensajeError(err));
        throw err;
      })
